...
export default {
  ...
  methods: {
    refresh: function() {
      ...
      // this.totalPage = Math.ceil(this.pager.total / this.pager.limit);
    ...
    getBlogsByTagCount(tagId) {
      axios({
        url: "/api/getBlogsByTagMappingCount?tagId=" + tagId,
        method: "get"
      }).then(resp => {
        this.$store.commit("changePager", { total: resp.data.data[0].count });
        // this.pager.total = resp.data.data[0].count;
    ...
    getBlogsByTag() {
      axios({
        url:
          "/api/queryBlogsByTagsByPage?tagId=" +
          this.tagId +
          "&offset=" +
          (this.pager.nowPage - 1) * this.pager.limit +
          "&limit=" +
  ...
  created() {
    ...
    // this.$root.$on("pageShow", showPage => {
    //   this.pager.display = showPage;
    // });
    // if (this.$route.params.tagId) {
    // this.tagId = this.$route.query.tagId;
    // this.$store.commit('changeTagId', this.$route.)
    // }
    // 设置blog总数
    if (this.tagId == "") {
      this.getBlogsCount();
